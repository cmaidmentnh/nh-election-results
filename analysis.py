...
COUNTY_OFFICE_SLUG = {8: 'county-sheriff', 9: 'county-attorney', 10: 'county-treasurer',
                      11: 'register-of-deeds', 12: 'register-of-probate', 13: 'county-commissioner'}


def get_county_office_races(county, year=None):
    """All COUNTY-office races (Sheriff, Attorney, Treasurer, Commissioner-by-district,
    Registers) for one county in a year — so a county can be viewed across offices.
    Each race carries the slug/district for linking to its detail page."""
    conn = get_connection()
    cur = conn.cursor()
    if year is None:
        cur.execute("""SELECT MAX(e.year) y FROM races r JOIN elections e ON e.id=r.election_id
                       WHERE r.county=? AND r.office_id IN (8,9,10,11,12,13) AND e.election_type='general'""",
                    (county,))
        row = cur.fetchone()
    ...
    if not year:
        ...
        return None
    cur.execute("""
        SELECT o.name office, r.office_id oid, r.district, r.seats,
               c.name, c.party, SUM(res.votes) v,
               RANK() OVER (PARTITION BY r.id ORDER BY SUM(res.votes) DESC) rk
        FROM results res JOIN candidates c ON c.id=res.candidate_id
        JOIN races r ON r.id=res.race_id JOIN elections e ON e.id=r.election_id
        JOIN offices o ON o.id=r.office_id
        WHERE r.county=? AND r.office_id IN (8,9,10,11,12,13) AND e.year=? AND e.election_type='general'
          AND c.name NOT IN ('Undervotes','Overvotes','Write-Ins')
        GROUP BY r.id, c.id
        ORDER BY r.office_id, CAST(r.district AS INTEGER), v DESC
    """, (county, year))
    races = {}
    for row in cur.fetchall():
        key = (row['oid'], row['district'] or '')
        if key not in races:
            races[key] = {'office': row['office'], 'slug': COUNTY_OFFICE_SLUG.get(row['oid']),
                          'district': row['district'] or '', 'seats': row['seats'], 'candidates': []}
        races[key]['candidates'].append({'name': row['name'], 'party': row['party'],
                                         'votes': row['v'], 'is_winner': row['rk'] <= row['seats']})
    # years available for this county's offices
    cur.execute("""SELECT DISTINCT e.year FROM races r JOIN elections e ON e.id=r.election_id
                   WHERE r.county=? AND r.office_id IN (8,9,10,11,12,13) AND e.election_type='general'
                   ORDER BY e.year DESC""", (county,))
